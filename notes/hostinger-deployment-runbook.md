...
The checked build is archived once with a commit/file-hash manifest. Before publishing, the workflow downloads a complete site snapshot and persists an AES-256-GCM encrypted rollback artifact. Files are staged and read back over verified FTPS; assets are promoted before documents. HTTP content hashes and CRM refresh routes must then pass. Raster images use `Cache-Control: no-transform` because they are already optimized in the build; this was verified through a temporary public-domain probe. A cached, losslessly reencoded image must match the exact original hash over verified origin HTTPS and have identical full-resolution decoded pixels. HTML and JavaScript always require exact public hashes. A failed promotion triggers restoration and quarantines newly introduced files.
